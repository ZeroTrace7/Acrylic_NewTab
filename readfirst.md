...
- Major directories: `/modules` (core logic), `/panels` (tools panel modules), `/settings` (Preferences UI), `/onboarding` (onboarding flow).
...
- Bottom-Right Group (List Icon, `#settings-btn`): Triggers "Preferences" modal.
